...
# MQTT 설정
mqtt_broker = "192.168.110.188"  # 예: "localhost" 또는 브로커의 올바른 호스트 이름/주소
mqtt_port = 1883
...
try:
    while True:
        ...
        if distance3 <= 5:
            client.publish(mqtt_topic_led3,"T")
            for i in range(8):
                for j in range(8):
                    sense.set_pixel(i, j, red)
        else:
            client.publish(mqtt_topic_led3,"F")
            for i in range(8):
                for j in range(8):
        ...
        #client.puclish(mqtt_topic_distance2, round(distance2,2))
